...
def create_app(config_object=None, testing_config=None):
    ...
    with app.app_context():
        try:
            ...
            Role = User = LoanProduct = LoanApplication = Account = Transaction = JournalEntry = Cliente = ContratoIntegracion = ProductoCredito = Empleado = Planilla = ClientProfile = Tenant = AuditLog = Payment = NotificationTemplate = Employee = Student = Course = Enrollment = Grade = TuitionPayment = MockModel
            app.services = {'audit_service': lambda: None, 'school_management_service': lambda: None}
