...
from models import db, Role, User, LoanProduct, LoanApplication, Account, Employee, PayrollLog, PaySlip, Lead, CommunicationLog, Payment, NotificationTemplate, AuditLog, Opportunity, MailingList, Campaign, Ticket, TicketComment, Project, Task, Flow
...
from datetime import datetime, date, timedelta
    ...
    def download_contract_pdf(app_id):
        ...
        contract_data = {
            ...
            "client": {
                ...
                "dui": application.applicant.dui,
                "nit": application.applicant.nit,
                "email": application.applicant.email # Needed for validation text
            },
            "company": company_info,
            "amortization_table": amortization_table,
            "tea_annual": loan_calculation['tea_annual']
        ...
        if application.signature_image and application.signed_at:
            contract_data['signature'] = {
                'image_b64': application.signature_image,
                'signed_at': application.signed_at.strftime('%Y-%m-%d %H:%M:%S UTC')
            }
    ...
    def update_opportunity(opp_id):
        ...
        if 'stage' in data:
            ...
            # --- Event Dispatch: Trigger for Flow ---
            if new_stage == 'Ganada' and old_stage != 'Ganada':
                event_service.dispatch('OPPORTUNITY_WON', data={'opportunity_id': opp.id})
    ...
    @app.route('/api/applications/<int:application_id>/sign', methods=['POST'])
    @jwt_required()
    def sign_application(application_id):
        app = LoanApplication.query.get_or_404(application_id)
        current_user = User.query.filter_by(email=get_jwt_identity()).first()

        # Security: only the loan applicant can sign
        if app.user_id != current_user.id:
            return jsonify({"message": "No está autorizado para firmar este contrato."}), 403

        if app.signature_image:
            return jsonify({"message": "Este contrato ya ha sido firmado."}), 400

        data = request.get_json()
        if 'signature_image' not in data:
            return jsonify({"message": "No se proporcionó la imagen de la firma."}), 400

        app.signature_image = data['signature_image']
        app.signed_at = datetime.utcnow()

        ip_address = request.remote_addr
        audit_details = f"Contrato para préstamo ID {app.id} firmado digitalmente por {current_user.email} desde la IP {ip_address}."
        audit_service.log_action('CONTRACT_SIGNED', user_id=current_user.id, details=audit_details)

        db.session.commit()

        return jsonify({"message": "Contrato firmado exitosamente."})
    ...
    @jwt_required()
    def handle_projects():
        # Accessible to all internal staff, but not clients
        claims = get_jwt()
        user_roles = claims.get('roles', [])
...
def setup_database(app):
    ...
    with app.app_context():
        ...
        if not Flow.query.first():
            new_flow = Flow(
                name='Convertir Oportunidad Ganada en Préstamo',
                trigger_event='OPPORTUNITY_WON',
                action_to_perform='CREATE_DRAFT_APPLICATION'
            )
