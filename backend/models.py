...
class LoanApplication(db.Model):
    ...
    signature_image = db.Column(db.Text, nullable=True) # To store the base64 encoded signature
...
class Project(db.Model):
    ...
    status = db.Column(db.String(50), default='Activo') # 'Activo', 'Archivado', 'Completado'
...
class Task(db.Model):
    ...
    description = db.Column(db.Text, nullable=True)
    status = db.Column(db.String(50), default='Pendiente') # 'Pendiente', 'En Progreso', 'Hecho'
    ...
    def to_dict(self):
        return {
            ...
            'assignee_name': self.assignee.full_name if self.assignee else 'Sin asignar'
        }

# --- Automation / Flow Models ---

class Flow(db.Model):
    """Defines an automation rule (Trigger -> Action)."""
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(255), nullable=False)
    trigger_event = db.Column(db.String(100), nullable=False, unique=True) # e.g., 'OPPORTUNITY_WON'
    action_to_perform = db.Column(db.String(100), nullable=False) # e.g., 'CREATE_DRAFT_APPLICATION'
    is_active = db.Column(db.Boolean, default=True)