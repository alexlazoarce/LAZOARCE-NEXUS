...
class FixedAsset(db.Model):
    ...
    depreciation_method = db.Column(String(50), default='linea_recta', nullable=False)
    status = db.Column(String(50), default='Activo', index=True)
    tenant_id = db.Column(Integer, ForeignKey('tenant.id'), nullable=False, index=True)
    created_at = db.Column(DateTime, default=func.current_timestamp())
...
class BankTransaction(db.Model):
    ...
    amount = db.Column(Float, nullable=False)
    transaction_type = db.Column(String(50))
...
class CashTransaction(db.Model):
    ...
    amount = db.Column(Float, nullable=False)
    tenant_id = db.Column(Integer, ForeignKey('tenant.id'), nullable=False, index=True)
    cash_box = db.relationship('CashBox', backref='transactions')
    def __repr__(self):
...
class TaxType(db.Model):
    ...
    id = db.Column(Integer, primary_key=True)
    name = db.Column(String(100), nullable=False, index=True)
    rate = db.Column(Float, nullable=False)
    country_code = db.Column(String(3), index=True)
    tax_category = db.Column(String(50), index=True)
    tenant_id = db.Column(Integer, ForeignKey('tenant.id'), nullable=False, index=True)
    is_active = db.Column(Boolean, default=True)
...
class ProgressReport(db.Model):
    ...
    percentage_complete = db.Column(Float, nullable=False)
    notes = db.Column(Text)
...
class AuditLog(db.Model):
    ...
    entity = db.Column(String(100), nullable=False)
