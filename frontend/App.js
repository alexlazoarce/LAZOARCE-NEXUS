function App() {
    ...
    const renderView = () => {
        ...
        switch (view) {
            ...
            case 'support': return isInternalStaff ? <SupportDashboardView token={token} /> : <ClientTicketsView token={token} />;
            case 'projects': return isInternalStaff ? <ProjectPortalView token={token} /> : <p>Acceso no autorizado.</p>;
            case 'accounting': return (isAdmin || userRoles.includes('Contador')) ? <AccountingPortal /> : <p>Acceso no autorizado.</p>;
            case 'hr': return isAdmin ? <HRPortal /> : <p>Acceso no autorizado.</p>;
            case 'crm': return (isAdmin || userRoles.includes('Ejecutivo de Crédito')) ? <CRMPortal /> : <p>Acceso no autorizado.</p>;
            case 'collections': return (isAdmin || userRoles.includes('Cobrador')) ? <PortfolioView token={token} /> : <p>Acceso no autorizado.</p>;
    ...
    const NavigationView = () => {
        ...
        return (
            <nav>
                {isInternalStaff && <button onClick={() => setView('dashboard')}>Dashboard</button>}
                {isClient && <button onClick={() => setView('dashboard')}>Mis Solicitudes</button>}

                {isInternalStaff && <button onClick={() => setView('projects')}>Proyectos</button>}
                <button onClick={() => setView('support')}>Soporte</button>

                {(isAdmin || userRoles.includes('Ejecutivo de Crédito')) && <button onClick={() => setView('crm')}>CRM</button>}
                {(isAdmin || userRoles.includes('Cobrador')) && <button onClick={() => setView('collections')}>Cobranza</button>}
                ...
                {isClient && <button onClick={() => setView('newApplication')}>Nueva Solicitud</button>}

                {(isAdmin || userRoles.includes('Contador')) && <button onClick={() => setView('accounting')}>Contabilidad</button>}
